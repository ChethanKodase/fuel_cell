...
void AirCompressor::AircompressorState(double clearnaceLength, double crankRadius, double connectingRodLength, double* time, double cylCrossSection, double copressorRPM, double atmPressure, double deliveryPressure, double storageTankVolume, double* currentVolume, double* internalPressure, double* pistonVelocity, double* nthRotation, double* massOutFlowRate, double atmTemperature, double stepSize, double valveCS, double* counter, double* powerDeveloped, double* powerConsumed) {
	...
	double gasDensityInAtmosphere = (atmPressure * M_air / (R * (atmTemperature + 273)));
	...
	*/
	...
	double maximumVolume = cylCrossSection * (clearnaceLength + 2.0 * crankRadius) ;
	...
	double durationLeft = (60.0 / (2.0 * (22.0 / 7.0) * copressorRPM)) * acos((crankRadius * crankRadius + ex * ex - connectingRodLength * connectingRodLength) / (2.0 * crankRadius * ex));
	...
	*pistonVelocity = (crankRadius * ((3.14285 * copressorRPM) / 30) * cos(((3.14285 / 2.0) + ((3.14285 * copressorRPM * (*time - (60 / copressorRPM) * (*nthRotation))) / 30)))) +
	...
	if (*pistonVelocity < 0) {
		...
		*internalPressure = atmPressure;
		...
		compressedGasDensity = (*internalPressure * M_air / (R * (compressedGasTemperature)));
	...
	double deliveryDensity = ((deliveryPressure /*+ atmPressure * 0.1*/) * M_air / (R * (deliveryTemperature)));
...
/*
double* AirCompressor::CylinderVolume(double clearnaceLength, double crankRadius, double connectingRodLength, double timeStepSize, double NoOfTimeSteps, double cylCrossSection, double copressorRPM, double atmPressure, double deliveryPressure, double storageTankVolume) {
	...
	double* currentVOlume = new double[NoOfTimeSteps];

	double* pistonVelocity = new double[NoOfTimeSteps];

	double* timeArray = new double[NoOfTimeSteps];

	double* pressureArray = new double[NoOfTimeSteps];
	...
	for (int i = 0; i < NoOfTimeSteps; i++) {
		...
		//currentVOlume[i] = cylCrossSection * (   clearnaceLength + crankRadius - crankRadius * sin(((3.14285/2) + ((3.14285*copressorRPM*time) / 30))  )   ) ;

		currentVOlume[i] = cylCrossSection * (clearnaceLength + connectingRodLength + crankRadius - crankRadius * sin(((3.14285 / 2) + ((3.14285 * copressorRPM * time) / 30))) - sqrt(((connectingRodLength * connectingRodLength) - (crankRadius * crankRadius) * (cos(((3.14285 / 2) + ((3.14285 * copressorRPM * time) / 30)))) * (cos(((3.14285 / 2) + ((3.14285 * copressorRPM * time) / 30)))))));


		//pistonVelocity[i] = (crankRadius * ((3.14285 * copressorRPM) / 30) * cos(((3.14285 / 2) + ((3.14285 * copressorRPM * time) / 30))));
		...
		if (pistonVelocity[i] <= 0) {

			pressureArray[i] = atmPressure;

		}
		else if (pistonVelocity[i] > 0) {

			maxVolume = cylCrossSection * (clearnaceLength + 2*crankRadius) ;

			instantPressure = atmPressure * pow((maxVolume/ currentVOlume[i]), polytropicIndex);


			if (instantPressure <= deliveryPressure) {

				pressureArray[i] = instantPressure ;

			}
			else {
				pressureArray[i] = deliveryPressure;
			}
...
double* AirCompressor::CompressorTime(double clearnaceLength, double crankRadius, double connectingRodLength, double timeStepSize, double NoOfTimeSteps, double cylCrossSection, double copressorRPM, double atmPressure, double deliveryPressure, double storageTankVolume) {
...
double* AirCompressor::CompressorPistonVelocity(double clearnaceLength, double crankRadius, double connectingRodLength, double timeStepSize, double NoOfTimeSteps, double cylCrossSection, double copressorRPM, double atmPressure, double deliveryPressure, double storageTankVolume) {
...
double* AirCompressor::CompressorInternalPressure(double clearnaceLength, double crankRadius, double connectingRodLength, double timeStepSize, double NoOfTimeSteps, double cylCrossSection, double copressorRPM, double atmPressure, double deliveryPressure, double storageTankVolume) {
